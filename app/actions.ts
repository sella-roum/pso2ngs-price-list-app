...
export async function fetchLatestProducts(
  ...
  try {
    const validatedParams = searchParamsSchema.parse({ category, productName, groupName });
...
export async function fetchProductHistory(productName: string): Promise<ProductRecord[]> {
  try {
    const validatedParams = productHistoryParamsSchema.parse({ productName });
    const supabase = createServerSupabaseClient();
...
export async function fetchProductAnalysisData(
  ...
  try {
    const validatedParams = analysisParamsSchema.parse({ targetType, targetName, period });
    const supabase = createServerSupabaseClient();
