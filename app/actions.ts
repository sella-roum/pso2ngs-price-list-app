...
if (!supabaseUrl || !supabaseAnonKey) {
  ...
  throw new Error('Supabase URL または Anon キーが設定されていません。環境変数を確認してください。')
}

const supabase = createClient(supabaseUrl, supabaseAnonKey)

export async function fetchLatestProducts(category: string, productName: string, groupName: string): Promise<ProductRecord[]> {
...
export async function fetchMaxLastModifiedRecord(): Promise<ProductRecord[]> {
  const { data, error } = await supabase
    .rpc('search_products_of_last_modified_date')
    .select('*')
